...
export const isOrderEmailConfigured = () =>
  Boolean(process.env.RESEND_API_KEY && process.env.ORDER_FROM_EMAIL);

export async function sendOrderConfirmationEmail(order: EmailOrder, attempt: number) {
...
聯絡信箱：${process.env.ORDER_ADMIN_EMAIL ?? "loxa8858@gmail.com"}`;
  ...
  if (!response.ok || !result.id) {
    throw new Error(result.message ?? result.error?.message ?? "確認信寄送失敗，訂單狀態尚未變更。");
  }
  return result.id;
}
