#!/usr/bin/env python3
"""Render the keychain opener Reel with original ambient audio."""
...
def synthesize_soundtrack(path: Path) -> None:
    ...
    chords = [
        (0.0, (164.81, 196.00, 246.94, 329.63)),
        (2.5, (146.83, 185.00, 220.00, 293.66)),
        (5.0, (130.81, 174.61, 220.00, 261.63)),
        (7.5, (146.83, 196.00, 246.94, 293.66)),
    ]
    for chord_index, (start, notes) in enumerate(chords):
        ...
        for sample_index in range(first, last):
            ...
            for voice_index, frequency in enumerate(notes):
                phase = chord_index * 0.17 + voice_index * 0.21
                tone = math.sin(2.0 * math.pi * frequency * t + phase)
                tone += 0.14 * math.sin(2.0 * math.pi * frequency * 2.0 * t + phase)
    ...
    for note_index, frequency in enumerate((329.63, 392.00, 493.88, 587.33) * 4):
        ...
        first = int(start * SAMPLE_RATE)
        last = min(total, int((start + 1.05) * SAMPLE_RATE))
        pan = 0.40 if note_index % 2 == 0 else 0.60
        for sample_index in range(first, last):
            local = sample_index / SAMPLE_RATE - start
            pluck = math.exp(-3.6 * local) * min(1.0, local / 0.02)
            value = math.sin(2.0 * math.pi * frequency * local) * pluck * 0.039
            left[sample_index] += value * math.sqrt(1.0 - pan)
            right[sample_index] += value * math.sqrt(pan)
...
def render_reel(image_path: Path, audio_path: Path, output_path: Path) -> None:
    ...
    filter_complex = (
        ...
        "zoompan=z='min(zoom+0.00024,1.028)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
        "d=96:s=1080x1920:fps=30[v0];"
        "[middle]crop=1080:1920:347:0,trim=duration=3.2,setpts=PTS-STARTPTS,"
        "zoompan=z='min(zoom+0.0002,1.024)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
        "d=96:s=1080x1920:fps=30[v1];"
        "[right]crop=1080:1920:694:0,trim=duration=3.6,setpts=PTS-STARTPTS,"
        ...
        f"{text_filter('base', 'txt0', '鑰匙圈就是開瓶器', 128, 0.25, 2.9)};"
        f"{text_filter('txt0', 'txt1', '瓶罐都能快速打開', 128, 3.15, 5.75)};"
        f"{text_filter('txt1', 'v', '私訊 開瓶鑰匙圈 加尺寸或照片', 1460, 7.0, 9.55)}"
    )
    command = [
        ffmpeg,
        "-hide_banner",
...
def main() -> None:
    ...
    root = here.parents[2]
    image_path = root / "public" / "lifestyle" / "keychain-opener-scenes.webp"
    audio_path = here / "15-keychain-opener-original-ambient.wav"
    output_path = here / "15-keychain-opener-reel.mp4"
