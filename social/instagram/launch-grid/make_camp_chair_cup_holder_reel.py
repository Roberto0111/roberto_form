#!/usr/bin/env python3
"""Render the camp chair cup holder Reel with original ambient audio."""
...
def synthesize_soundtrack(path: Path) -> None:
    ...
    chords = [
        (0.0, (146.83, 196.00, 246.94, 293.66)),
        (2.5, (164.81, 207.65, 261.63, 329.63)),
        (5.0, (130.81, 196.00, 220.00, 293.66)),
        (7.5, (146.83, 185.00, 246.94, 311.13)),
    ]
    for chord_index, (start, notes) in enumerate(chords):
        ...
        for sample_index in range(first, last):
            ...
            for voice_index, frequency in enumerate(notes):
                phase = chord_index * 0.15 + voice_index * 0.23
                tone = math.sin(2.0 * math.pi * frequency * t + phase)
                tone += 0.12 * math.sin(2.0 * math.pi * frequency * 2.0 * t + phase)
                value = tone * envelope * 0.023
                pan = 0.28 + (voice_index / max(1, len(notes) - 1)) * 0.44
                left[sample_index] += value * math.sqrt(1.0 - pan)
                right[sample_index] += value * math.sqrt(pan)

    for note_index, frequency in enumerate((293.66, 369.99, 440.00, 493.88) * 4):
        start = 0.35 + note_index * 0.52
        first = int(start * SAMPLE_RATE)
        last = min(total, int((start + 1.0) * SAMPLE_RATE))
        pan = 0.42 if note_index % 2 == 0 else 0.58
        for sample_index in range(first, last):
            local = sample_index / SAMPLE_RATE - start
            pluck = math.exp(-3.7 * local) * min(1.0, local / 0.02)
            value = math.sin(2.0 * math.pi * frequency * local) * pluck * 0.036
            left[sample_index] += value * math.sqrt(1.0 - pan)
            right[sample_index] += value * math.sqrt(pan)

    dry_left = array("f", left)
    dry_right = array("f", right)
    for delay_seconds, gain in ((0.18, 0.16), (0.36, 0.09)):
...
def render_reel(image_path: Path, audio_path: Path, output_path: Path) -> None:
    ...
    filter_complex = (
        "[0:v]scale=3840:1920,format=yuv420p,split=3[left][middle][right];"
        ...
        "[middle]crop=1080:1920:1380:0,trim=duration=3.2,setpts=PTS-STARTPTS,"
        "zoompan=z='min(zoom+0.0002,1.024)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
        "d=96:s=1080x1920:fps=30[v1];"
        "[right]crop=1080:1920:2760:0,trim=duration=3.6,setpts=PTS-STARTPTS,"
        "zoompan=z='min(zoom+0.00022,1.026)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
        ...
        f"{text_filter('base', 'txt0', '露營椅旁邊就能放飲料', 128, 0.25, 2.9)};"
        f"{text_filter('txt0', 'txt1', '杯子 小盤 零食不落地', 128, 3.15, 5.75)};"
        f"{text_filter('txt1', 'v', '私訊 露營杯架 加椅管尺寸或照片', 1400, 7.0, 9.55)}"
    )
    command = [
        ffmpeg,
        "-hide_banner",
...
def main() -> None:
    ...
    root = here.parents[2]
    image_path = root / "public" / "lifestyle" / "camp-chair-tray-scenes.webp"
    audio_path = here / "17-camp-chair-cup-holder-original-ambient.wav"
    output_path = here / "17-camp-chair-cup-holder-reel.mp4"
